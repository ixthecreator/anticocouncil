...
首次使用默认「议会紫」，已有四种主题偏好继续保留；可在设置中切换。原有八个业务模块与锚点、登录审批、云端保存、投票签到和导入导出继续沿用。UI PR #1 已于 2026 年 9 月 10 日按用户要求合并至 main，并由 Vercel 发布。
...
已合并的 UI 包含统一的深紫页眉、公开首页、按届次排列的档案馆和静态文章页。首页保留协作工作台及 `/preview/` 入口；`/blog` 兼容旧地址。当前届次和文章均明确标为设计示例，没有真实往届材料，也不读取内部会议、成员或选票。

公开页面在构建时生成独立 HTML，不运行应用脚本，不加载 Google 字体。真实稿件可在仓库中维护，无需新后台。内容规范、静态构建和路由说明见 [公开档案馆维护说明](docs/public-archive.md)。访问路径：`/`、`/archive`、`/archive/sample-working-records`。

功能 PR #4（PDF 修复、Word 导出、会议改名）和 UI PR #1 都已合并至 main。首页、档案馆与工作台现在使用同一设计语言；静态文章尚为明确标注的排版示例。

## 功能修复与私密投票（部署准备中）

本分支增加服务端私密投票、本人回执及零票取消，并修复多人编辑覆盖、旧表单复活已删除记录、切换账号后旧写入继续执行、会议删除遗留关联记录和云端导入验证问题。工作台也改用本机字体，不再请求 Google Fonts；Firebase 认证及数据库仍保持现有项目，未迁移 Supabase。

私密投票必须同时部署 Vercel `/api/voting` 和 `firestore.rules`，并安全转存旧选票后才生效。当前代码验证不代表正式环境已配置服务端凭据或完成旧记录迁移。具体配置、权限、旧票处理、验收及回滚见 [私密投票部署说明](docs/private-voting-deployment.md)。Node 运行时要求 22，服务账号凭据只放服务端变量，绝不能加 `VITE_` 前缀或提交到仓库。

本地试用演示不可改票和结束后汇总；本地浏览器存储不具备账号级隐私。普通 JSON 备份包含八类业务数据，不含云端私票集合，不能作为完整私票灾备。云端导入保持已有议题的状态、关联会议、归档状态与表决记录，已投票命题不会被旧备份改写；新编号的表决记录需要受控恢复，不能通过普通导入重新创建。
...
试用操作仅保存到独立的 `antico-ui-preview-v1` 浏览器存储；重置只清除此键。该页面不加载 Firebase、不读写正式工作区数据，网络连接受独立页面策略禁止。生产上的 `/portal`、`/workspace` 和 `/local` 已使用新 React UI。
...
- 新文档不含逐人选票；纪要只列已结束表决的汇总，议程不列票数。PDF/Word 导出本身不等于投票隐私保护；私密投票须按下方服务端部署说明启用。
...
共享快照和格式实现位于 `src/lib/meetingExport.ts`、`meetingPdf.ts` 与 `meetingDocx.ts`。字体来源、许可证和重建方法见 `public/fonts/README.md`。Supabase 迁移继续延后；`/preview/` 保持原示例体验。
...
登录审批控制进入共享工作区的资格。新私密投票按登录账号 UID 绑定，云端管理员发起和结束；业务成员名册与签到仍按现有成员记录维护，没有自动与登录账号绑定。
...
会议流程为：新建或选择例会 → 添加成员并签到 → 记录待汇报/已汇报/免汇报 → 讨论与表决 → 授权执行 → 完成归档。历史汇报按会议日期计算，免汇报需填写原因，不自动写死周三/周日规则。新云端表决按已获批的登录账号 UID 计一票，提交后不可修改，结束后公开匿名汇总。旧主持人录票仅供迁移保留，不再新增这种表决。

资料库支持 PNG/JPG/WEBP/PDF 附件，单文件最多 400 KB，存入当前工作区业务数据；大文件使用 HTTP(S) 链接。同名作者资料需要自行区分。外部表格仅保留业务入口，未抓取或迁移其内容。

## 验证与发布

本次功能分支通过 TypeScript、219 项测试（1138 个断言）、26 项真实 Firestore Emulator 测试、生产构建及 6 个静态公开页面检查。已完成本地投票流程验证，正式私票发布仍待服务端凭据与旧票迁移。[完整代码检查与必要功能建议](docs/code-audit-2026-09-10.md)。
